...
export default function AdminIdentidadView() {
  ...
  return (
    <div className="w-full space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      <div className="w-full bg-white rounded-3xl p-6 md:p-8 border border-rose-100 shadow-[0_4px_24px_rgba(225,29,72,0.04)] space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-rose-100 pb-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <span className="text-[9px] font-black uppercase tracking-[0.25em] text-rose-600 block mb-0.5">
                Mantenimiento de Base de Datos
              </span>
              <h3 className="text-lg font-display font-black text-slate-800 tracking-tight leading-none">
          ...
          <div className="flex items-center gap-2 bg-slate-50 border border-slate-200/80 px-4 py-2.5 rounded-2xl flex-shrink-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="bg-rose-50/50 border border-rose-100 p-3.5 rounded-2xl">
            <span className="text-[9px] font-black uppercase text-rose-500 block mb-1">Pedidos Registrados</span>
            <span className="text-lg font-black text-slate-800 font-mono">{ordersCountToday}</span>
            <span className="text-[9px] text-slate-400 font-bold block mt-0.5">comprobantes / comandas</span>
          </div>

          <div className="bg-rose-50/50 border border-rose-100 p-3.5 rounded-2xl">
            <span className="text-[9px] font-black uppercase text-rose-500 block mb-1">Apertura de Caja</span>
          ...
          <div className="bg-rose-50/50 border border-rose-100 p-3.5 rounded-2xl">
            <span className="text-[9px] font-black uppercase text-rose-500 block mb-1">Transacciones Clientes</span>
            <span className="text-lg font-black text-slate-800 font-mono">{customerTxCountToday}</span>
            <span className="text-[9px] text-slate-400 font-bold block mt-0.5">abonos / fiados hoy</span>
          </div>
        </div>

        {deleteSuccess && (
          <div className="flex items-center gap-2 bg-emerald-50 text-emerald-800 px-4 py-3 rounded-2xl border border-emerald-100 animate-in fade-in duration-300">
            <CheckCircle2 className="w-4 h-4 text-emerald-500 flex-shrink-0" />
        ...
        {deleteError && (
          <div className="flex items-center gap-2 bg-rose-50 text-rose-800 px-4 py-3 rounded-2xl border border-rose-200 animate-in fade-in duration-300">
        ...
        <div className="flex items-center justify-between pt-2">
          <p className="text-[10px] text-slate-400 font-medium italic">
            ⚠️ Atención: Esta acción borrará permanentemente los datos listados para la fecha <strong className="text-slate-600 font-bold">{selectedDate}</strong>.
          </p>
          ...
          <button
            ...
            className="px-5 py-3.5 bg-rose-600 hover:bg-rose-700 active:scale-98 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-rose-200 transition-all flex items-center gap-2"
          >
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 animate-in fade-in duration-200">
          ...
          <div className="relative bg-white w-full max-w-md rounded-[32px] p-6 md:p-8 shadow-2xl border border-slate-100 space-y-5">
            <div className="flex items-center gap-3">
              <div className="p-3 bg-rose-100 text-rose-600 rounded-2xl">
                <Trash2 className="w-6 h-6" />
              </div>
              <div>
                <h4 className="text-lg font-black text-slate-800 uppercase tracking-tight">Confirmar Purga de Datos</h4>
                <p className="text-xs font-bold text-rose-600 uppercase tracking-wider">Fecha: {selectedDate}</p>
              </div>
            </div>

            <div className="bg-rose-50 border border-rose-100 rounded-2xl p-4 text-xs text-rose-900 space-y-2 leading-relaxed">
              <p className="font-bold">
                ¿Estás seguro de que deseas eliminar permanentemente los registros del día <strong>{selectedDate}</strong>?
              </p>
              <ul className="list-disc list-inside space-y-1 text-[11px] font-medium text-rose-800">
              ...
              <p className="text-[10px] font-black uppercase tracking-wider text-rose-700 pt-1">
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              <button
                ...
                disabled={isDeletingDay}
                className="py-3.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-2xl text-[10px] font-black uppercase tracking-wider transition-all"
              >
                Cancelar
              </button>
              ...
              <button
                ...
                disabled={isDeletingDay}
                className="py-3.5 bg-rose-600 hover:bg-rose-700 active:scale-98 text-white rounded-2xl text-[10px] font-black uppercase tracking-wider shadow-lg shadow-rose-200 transition-all flex items-center justify-center gap-2"
              >
                {isDeletingDay ? (
                  <>
