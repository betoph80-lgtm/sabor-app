...
export default function AdminUsuariosView() {
  ...
  const getRoleTheme = (role: Role) => {
    switch (role) {
      case 'ADMIN':
        return { text: 'text-brand-700 font-extrabold', bg: 'bg-brand-100 border-brand-200', gradient: 'from-brand-500 to-indigo-500' };
      case 'CAJA':
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {appUsers.map(user => {
            ...
            return (
              <div 
                key={user.id} 
                className="bg-white rounded-[24px] p-5 border border-slate-150/80 shadow-xs hover:border-brand-300 hover:shadow-md transition-all duration-300 flex items-center justify-between group"
              >
                ...
                <div className="flex items-center gap-1 shrink-0 pl-2">
                  <button 
                    ...
                    className="p-2 hover:bg-brand-50 text-slate-400 hover:text-brand-600 rounded-xl transition-all cursor-pointer"
            ...
            <form onSubmit={editingUserId ? handleUpdateUserSubmit : handleCreateUser} className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <input 
                  ...
                  placeholder="****"
                  required
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-xs md:text-sm font-bold text-slate-800 focus:bg-white focus:border-brand-500 outline-none tracking-widest text-center transition-all"
                />
              </div>
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button 
                  type="submit" 
                  className="flex-2 bg-brand-600 hover:bg-brand-700 text-white rounded-xl py-3.5 font-black uppercase text-[10px] tracking-widest shadow-md shadow-brand-200 transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                >
