  ...
  return (
    <div className="space-y-6 md:space-y-8 w-full max-w-7xl mx-auto pb-16">
        ...
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-[10px] font-black uppercase tracking-wider shadow-sm flex items-center gap-1.5 transition-all active:scale-95 cursor-pointer"
              >
          ...
          <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
            <div className="flex flex-col md:flex-row md:items-center justify-between pb-4 border-b border-slate-100 gap-2">
              <div>
                <span className="px-2 py-0.5 bg-brand-50 text-brand-700 border border-brand-200 rounded-md text-[8.5px] font-black uppercase tracking-wider">
              ...
              <div className="text-right">
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block">Total Ventas</span>
                ...
                <tbody className="divide-y divide-slate-100 text-xs font-semibold">
                  {dailyConsolidated.asientoVentas.filas.map((fila, idx) => (
                    <tr key={idx} className="hover:bg-slate-50/60 transition-colors">
                      <td className="py-3 px-4 text-center font-mono font-black text-brand-700 bg-brand-50/30">
                        {fila.cuenta}
                      </td>
                  ...
                  <tr className="bg-slate-900 text-white font-black text-xs">
                    <td colSpan={2} className="py-3 px-4 text-right uppercase tracking-wider text-[10px] text-slate-300">
          ...
          <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
            <div className="flex flex-col md:flex-row md:items-center justify-between pb-4 border-b border-slate-100 gap-2">
              <div>
                <span className="px-2 py-0.5 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-md text-[8.5px] font-black uppercase tracking-wider">
              ...
              <div className="text-right">
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block">Total Cobrado</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-5">
            <div className="flex items-center gap-2.5 pb-3 border-b border-slate-100">
              ...
              <div>
                <h3 className="text-sm font-black text-slate-900 uppercase tracking-tight">Convertidor Contable SUNAT</h3>
                <p className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Ingresa una transacción para procesar</p>
              </div>
            </div>

            {/* Tipo de Operación */}
            <div className="space-y-1.5">
              <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Tipo de Transacción</label>
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between items-center">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Monto (Soles)</label>
                <label className="flex items-center gap-1.5 text-[9.5px] font-bold text-slate-600 cursor-pointer">
                  <input
                    type="checkbox"
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 pl-8 pr-3 font-mono font-black text-base outline-none focus:border-brand-500 transition-all text-slate-900"
                  placeholder="0.00"
                />
              </div>
            </div>
            ...
            {(interactiveMode === 'COMPRA' || interactiveMode === 'GASTO_SERVICIO') && (
              <div className="space-y-1.5">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Categoría del Gasto / Detracción</label>
                <select
                  value={interactiveCategory}
                  onChange={(e) => setInteractiveCategory(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 px-3 text-xs font-bold outline-none focus:border-brand-500 text-slate-800 cursor-pointer"
                >
            ...
            {interactiveMode === 'COBRANZA' && (
              <div className="space-y-1.5">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Medio de Pago Destino</label>
                <div className="grid grid-cols-2 gap-2">
                  <button
                    onClick={() => setInteractivePaymentMethod('EFECTIVO')}
                    className={`py-2 px-3 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all cursor-pointer ${
                      interactivePaymentMethod === 'EFECTIVO' ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-600'
                    }`}
                  >
                    Caja Efectivo (1011)
                  </button>
                  <button
                    ...
                    className={`py-2 px-3 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all cursor-pointer ${
                      interactivePaymentMethod === 'YAPE' ? 'bg-brand-600 text-white' : 'bg-slate-100 text-slate-600'
                    }`}
                  >
            ...
            {/* Glosa / Descripción */}
            <div className="space-y-1.5">
              <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Glosa / Concepto Contable</label>
              <input
                type="text"
                value={interactiveDescription}
                onChange={(e) => setInteractiveDescription(e.target.value)}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 px-3 text-xs font-semibold outline-none focus:border-brand-500 text-slate-800"
          ...
          <div className="lg:col-span-7 space-y-5">
            ...
            {liveAccountingEntry.alerts.length > 0 && (
              <div className="space-y-2">
                {liveAccountingEntry.alerts.map((al, idx) => (
                  <div key={idx} className="bg-amber-50 border border-amber-200/80 rounded-2xl p-3.5 text-xs text-amber-900 font-semibold flex items-start gap-2.5">
            ...
            <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
              <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                <div>
                  <span className="px-2 py-0.5 bg-brand-50 text-brand-700 border border-brand-200 rounded-md text-[8px] font-black uppercase tracking-wider">
                ...
                <div className="text-right">
                  <span className="text-[8.5px] font-black text-slate-400 uppercase tracking-widest block">Total Partida Doble</span>
                  ...
                  <tbody className="divide-y divide-slate-100 text-xs font-semibold">
                    {liveAccountingEntry.entry.filas.map((f, idx) => (
                      <tr key={idx} className="hover:bg-slate-50/60">
                        <td className="py-2.5 px-3 text-center font-mono font-black text-brand-700 bg-brand-50/20">
                          {f.cuenta}
                        </td>
                        <td className="py-2.5 px-3 text-slate-800 text-[11.5px]">
                          {f.denominacion}
                        </td>
                    ...
                    <tr className="bg-slate-900 text-white font-black text-xs">
                      <td colSpan={2} className="py-2.5 px-3 text-right uppercase tracking-wider text-[9px] text-slate-300">
              ...
              <div className="bg-slate-50 rounded-2xl p-4 border border-slate-150 space-y-2 mt-4">
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Resumen Tributario (SUNAT)</p>
                <div className="grid grid-cols-3 gap-3 text-center">
                  <div className="bg-white p-2.5 rounded-xl border border-slate-200">
                    <span className="text-[8px] font-black text-slate-400 uppercase block">Base Imponible</span>
                    <span className="text-sm font-mono font-bold text-slate-800">S/ {liveAccountingEntry.summary.baseImponible.toFixed(2)}</span>
                  </div>
                  <div className="bg-white p-2.5 rounded-xl border border-slate-200">
                    <span className="text-[8px] font-black text-slate-400 uppercase block">IGV (18%)</span>
                    <span className="text-sm font-mono font-bold text-brand-600">S/ {liveAccountingEntry.summary.igv.toFixed(2)}</span>
                  </div>
                  <div className="bg-white p-2.5 rounded-xl border border-slate-200">
                    <span className="text-[8px] font-black text-slate-400 uppercase block">Total Facturado</span>
                    <span className="text-sm font-mono font-bold text-emerald-700">S/ {liveAccountingEntry.summary.total.toFixed(2)}</span>
                  </div>
                </div>
                <p className="text-[10px] text-slate-500 font-mono mt-1 pt-2 border-t border-slate-200/60">
            ...
            <table className="w-full text-left border-collapse min-w-[900px]">
              ...
              <tbody className="divide-y divide-slate-100 text-xs font-semibold">
                {dailyOrders.map((order, idx) => {
                  ...
                  return (
                    <tr key={order.id} className="hover:bg-slate-50/60 transition-colors">
                      <td className="py-3 px-4 text-center font-mono font-bold text-slate-500 text-[11px]">
                      ...
                      <td className="py-3 px-4">
                        <span className="font-mono font-bold text-brand-700 bg-brand-50 px-2 py-0.5 rounded-md border border-brand-100">
                          {compNum}
                        </span>
                      </td>
                      <td className="py-3 px-4 uppercase text-slate-800 font-bold truncate max-w-[180px]">
                        {order.cliente}
                      </td>
                      ...
                      <td className="py-3 px-4 text-center">
                        <span className="px-2 py-0.5 rounded-full text-[8.5px] font-black uppercase tracking-wider bg-emerald-50 text-emerald-700 border border-emerald-200">
                          {order.estado}
                        </span>
                      </td>
                      <td className="py-3 px-4 text-center">
                        <button
                          ...
                          className="p-1.5 rounded-lg bg-slate-100 hover:bg-brand-50 hover:text-brand-600 transition-colors text-slate-600 cursor-pointer"
                ...
                {dailyOrders.length === 0 && (
                  <tr>
                    <td colSpan={9} className="py-12 text-center text-slate-400 font-bold uppercase tracking-widest text-xs">
              ...
              <tbody className="divide-y divide-slate-100 text-xs font-semibold">
                {purchases.map((purchase) => (
                  <tr key={purchase.id} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <p className="font-bold text-slate-800 uppercase truncate max-w-[200px]">{purchase.proveedor}</p>
                      <p className="text-[9px] font-mono text-slate-400">RUC: {purchase.rucProveedor}</p>
                    </td>
                    <td className="py-3 px-4">
                    ...
                    <td className="py-3 px-4">
                      <span className="px-2 py-0.5 rounded-md text-[8.5px] font-black uppercase tracking-wider bg-slate-100 text-slate-700">
                    ...
                    <td className="py-3 px-4 text-center">
                      <div className="flex justify-center gap-1">
                        {purchase.detraccionAplica && (
                          <span className="px-1.5 py-0.5 bg-amber-50 text-amber-700 border border-amber-200 rounded text-[7.5px] font-black uppercase">
                            SPOT {((purchase.tasaDetraccion || 0) * 100)}%
                          </span>
                        )}
                        {purchase.bancarizado && (
                          <span className="px-1.5 py-0.5 bg-sky-50 text-sky-700 border border-sky-200 rounded text-[7.5px] font-black uppercase">
                            BANC.
                          </span>
                        )}
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
              <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                <h3 className="text-sm font-black text-slate-800 uppercase tracking-tight flex items-center gap-2">
                  <Scale className="w-4 h-4 text-brand-600" /> 1. Impuesto General a las Ventas (IGV 18%)
                </h3>
              </div>

              <div className="space-y-3 text-xs">
                <div className="flex justify-between items-center py-2 border-b border-slate-100">
                  <span className="text-slate-600 font-semibold">Ventas Netas Gravadas (Base Imponible):</span>
                  <span className="font-mono font-bold text-slate-900">S/ {taxSettlement.ventasBase.toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-slate-100">
                  <span className="text-emerald-700 font-bold">Débito Fiscal IGV (18% de Ventas):</span>
                  <span className="font-mono font-black text-emerald-700">+ S/ {taxSettlement.ventasIgvDebito.toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-slate-100">
                  <span className="text-slate-600 font-semibold">Compras con Derecho a Crédito Fiscal:</span>
                  <span className="font-mono font-bold text-slate-900">S/ {taxSettlement.comprasBase.toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-slate-100">
                  <span className="text-rose-600 font-bold">Crédito Fiscal IGV (18% de Compras):</span>
                  <span className="font-mono font-black text-rose-600">- S/ {taxSettlement.comprasIgvCredito.toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center pt-2 text-sm bg-brand-50/50 p-3 rounded-xl border border-brand-100">
                  <span className="font-black text-brand-900 uppercase">IGV Resultante por Pagar:</span>
                  <span className="font-mono font-black text-brand-700 text-base">S/ {taxSettlement.igvPorPagar.toFixed(2)}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
              <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                <h3 className="text-sm font-black text-slate-800 uppercase tracking-tight flex items-center gap-2">
                  <Building2 className="w-4 h-4 text-brand-600" /> 2. Impuesto a la Renta (Pagos a Cuenta)
                </h3>
              </div>

              <div className="space-y-3 text-xs">
                <div className="flex justify-between items-center py-2 border-b border-slate-100">
                  <span className="text-slate-600 font-semibold">Régimen Tributario:</span>
                  <span className="font-bold text-brand-700 uppercase">{fiscalConfig.regimenTributario}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-slate-100">
                  <span className="text-slate-600 font-semibold">Tasa Pago a Cuenta:</span>
                  <span className="font-mono font-bold text-slate-900">{(taxSettlement.tasaRenta * 100).toFixed(1)}%</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-slate-100">
                  <span className="text-slate-700 font-bold">Pago a Cuenta de Renta del Periodo:</span>
                  <span className="font-mono font-black text-slate-900">S/ {taxSettlement.pagoCuentaRenta.toFixed(2)}</span>
                </div>
      ...
      {activeTab === 'CONFIG_FISCAL' && (
        <div className="bg-white rounded-3xl p-6 md:p-8 border border-slate-200 shadow-sm space-y-6 max-w-4xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div className="space-y-1.5">
              <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">RUC Empresa (11 Dígitos)</label>
              <input
                type="text"
                maxLength={11}
                value={fiscalConfig.ruc}
                onChange={(e) => saveFiscalConfig({ ...fiscalConfig, ruc: e.target.value })}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 px-3 font-mono font-bold text-sm outline-none focus:border-brand-500"
              />
            </div>

            <div className="space-y-1.5">
              <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Razón Social Legal</label>
              <input
                type="text"
                value={fiscalConfig.razonSocial}
                onChange={(e) => saveFiscalConfig({ ...fiscalConfig, razonSocial: e.target.value.toUpperCase() })}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 px-3 font-bold text-sm outline-none focus:border-brand-500 uppercase"
              />
            </div>

            <div className="space-y-1.5 md:col-span-2">
              <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Dirección Fiscal / Local</label>
              <input
                type="text"
                value={fiscalConfig.direccionFiscal}
                onChange={(e) => saveFiscalConfig({ ...fiscalConfig, direccionFiscal: e.target.value })}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 px-3 text-xs font-semibold outline-none focus:border-brand-500"
              />
            </div>

            <div className="space-y-1.5">
              <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Régimen Tributario</label>
              <select
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 px-3 text-xs font-bold outline-none focus:border-brand-500 cursor-pointer"
              >
            ...
            <div className="space-y-1.5">
              <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Valor UIT Vigente (2024)</label>
              <input
                type="number"
                value={fiscalConfig.uitVigente}
                onChange={(e) => saveFiscalConfig({ ...fiscalConfig, uitVigente: parseFloat(e.target.value) || 5150 })}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 px-3 font-mono font-bold text-xs outline-none focus:border-brand-500"
              />
            </div>

            <div className="space-y-1.5">
              <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Serie Boleta Electrónica</label>
              <input
                type="text"
                value={fiscalConfig.serieBoleta}
                onChange={(e) => saveFiscalConfig({ ...fiscalConfig, serieBoleta: e.target.value.toUpperCase() })}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 px-3 font-mono font-bold text-xs outline-none focus:border-brand-500 uppercase"
              />
            </div>

            <div className="space-y-1.5">
              <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Serie Factura Electrónica</label>
              <input
                type="text"
            ...
            >
              <div className="p-6 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-base font-black text-slate-800 uppercase tracking-tight">Registrar Compra / Factura</h3>
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Ingreso al Registro de Compras (PLE 8.1)</p>
                  </div>
                </div>
                <button onClick={() => setShowPurchaseModal(false)} className="p-2 hover:bg-slate-200/60 rounded-xl transition-colors">
                  ✕
                </button>
              </div>
              ...
              <form onSubmit={handleAddPurchase} className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">RUC Proveedor (11 Dígitos)</label>
                    <input
                      type="text"
                      maxLength={11}
                      required
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2 px-3 font-mono font-bold text-xs outline-none focus:border-brand-500"
                    />
                  </div>

                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Tipo Comprobante</label>
                    <select
                      value={purchaseForm.tipoComprobante}
                      onChange={(e) => setPurchaseForm({ ...purchaseForm, tipoComprobante: e.target.value as any })}
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2 px-3 text-xs font-bold outline-none focus:border-brand-500 cursor-pointer"
                    >
                ...
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Nombre o Razón Social Proveedor</label>
                  <input
                    type="text"
                    required
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2 px-3 text-xs font-bold outline-none focus:border-brand-500 uppercase"
                  />
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Serie y Número</label>
                    <input
                      type="text"
                      placeholder="F001-0001245"
                      value={purchaseForm.serieNumero}
                      onChange={(e) => setPurchaseForm({ ...purchaseForm, serieNumero: e.target.value })}
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2 px-3 font-mono font-bold text-xs outline-none focus:border-brand-500 uppercase"
                    />
                  </div>

                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Monto Total (S/)</label>
                    <input
                      type="number"
                      step="0.01"
                      required
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2 px-3 font-mono font-black text-sm outline-none focus:border-brand-500 text-slate-900"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Categoría del Gasto</label>
                    <select
                ...
                <div className="pt-3 flex gap-2">
                  <button
                    type="button"
                    onClick={() => setShowPurchaseModal(false)}
                    className="flex-1 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-black uppercase tracking-wider transition-colors cursor-pointer"
                  >
                    Cancelar
                  </button>
                  <button
        ...
        {previewOrder && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative bg-white w-full max-w-sm rounded-[32px] shadow-2xl overflow-hidden p-6 space-y-4"
            >
              ...
              <div className="text-center space-y-1 pb-3 border-b border-dashed border-slate-300">
                ...
                <p className="text-[10px] text-slate-500 font-mono">RUC: {fiscalConfig.ruc}</p>
                <p className="text-[9px] text-slate-400">{fiscalConfig.direccionFiscal}</p>
                <div className="pt-2">
                  <span className="px-3 py-1 bg-slate-900 text-white rounded-md text-[10px] font-mono font-black block w-fit mx-auto">
              ...
              <div className="space-y-1 text-xs text-slate-600 font-medium">
                <div className="flex justify-between">
                  ...
                  <span className="font-mono text-slate-800">{previewOrder.fecha} {previewOrder.hora}</span>
                </div>
                <div className="flex justify-between">
                  <span>Cliente:</span>
                  <span className="font-bold text-slate-800 uppercase">{previewOrder.cliente}</span>
                </div>
                <div className="flex justify-between">
                  <span>Mesa:</span>
                  <span className="text-slate-800 font-bold">{previewOrder.mesaId === '13' ? 'Para Llevar' : `Mesa ${previewOrder.mesaId}`}</span>
                </div>
              </div>
              ...
              <div className="border-t border-b border-dashed border-slate-300 py-2 space-y-1 text-xs">
                {previewOrder.items.map((item, i) => (
                  <div key={i} className="flex justify-between items-center text-[11px]">
                    <span>{item.cantidad}x Plato/Item</span>
                    <span className="font-mono font-bold text-slate-800">S/ {(item.cantidad * item.precioUnitario).toFixed(2)}</span>
                  </div>
                ))}
              </div>
                ...
                return (
                  <div className="space-y-1 text-xs font-semibold">
                    <div className="flex justify-between text-slate-500">
                      ...
                      <span className="font-mono">S/ {breakdown.baseImponible.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-slate-500">
                      <span>I.G.V. (18%):</span>
                      <span className="font-mono text-brand-600">S/ {breakdown.igv.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-sm font-black text-slate-900 pt-1 border-t border-slate-200">
                      <span>TOTAL A PAGAR:</span>
                      <span className="font-mono text-emerald-700">S/ {previewOrder.total.toFixed(2)}</span>
                    </div>
                    <p className="text-[9px] text-slate-400 font-mono mt-2 pt-2 border-t border-dashed border-slate-200 text-center">
