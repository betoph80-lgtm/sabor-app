...
import { 
  BarChart3, X, Utensils, Tag, Tags, Users, Compass, Settings, LayoutDashboard, FileText, Wallet, Scale 
} from 'lucide-react';
...
import AdminContabilidadSunatView from './components/AdminContabilidadSunatView.tsx';
...
const AdminPanel = () => {
  ...
  const tabs = [
    ...
    { id: 'CONTABILIDAD', label: 'Contabilidad SUNAT', icon: Scale },
  ...
  return (
    <>
      <div className="w-full max-w-7xl space-y-6 md:space-y-8 pb-10 md:pb-25">
        ...
        <div className="lg:hidden flex bg-slate-50/90 dark:bg-slate-900/90 border border-slate-200/60 dark:border-slate-800 backdrop-blur-md p-1 rounded-full shadow-[0_2px_15px_rgba(0,0,0,0.015)] overflow-x-auto no-scrollbar mx-auto max-w-fit sticky top-4 z-40">
        ...
        <div className="mt-8">
          ...
          {adminSubView === 'CONTABILIDAD' && <AdminContabilidadSunatView />}
        ...
        {showReport && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-lg rounded-[36px] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]"
            >
              {/* Modal Header */}
              <div className="p-6 md:p-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-850">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-emerald-50 dark:bg-emerald-950/60 rounded-2xl flex items-center justify-center text-emerald-600 dark:text-emerald-400 border border-emerald-100/50 dark:border-emerald-800/40">
                    <BarChart3 className="w-5 h-5" />
                  </div>
                  <div className="text-left">
                    <h3 className="text-lg font-display font-black text-slate-800 dark:text-white uppercase tracking-tight">Reporte de Ventas</h3>
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-0.5">{selectedDate}</p>
                  </div>
                </div>
                <button onClick={() => setShowReport(false)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-colors">
                  ...
                  return (
                    <div className="space-y-6">
                      <div className="grid grid-cols-2 gap-3.5">
                         <div className="bg-emerald-50/50 dark:bg-emerald-950/30 border border-emerald-100 dark:border-emerald-900/40 p-5 rounded-2xl text-left">
                           <p className="text-[8.5px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-wider mb-1">Efectivo Real</p>
                           <p className="text-xl font-display font-black text-emerald-900 dark:text-emerald-200 leading-none">S/ {efectivo.toFixed(2)}</p>
                         </div>
                         <div className="bg-sky-50/50 dark:bg-sky-950/30 border border-sky-100 dark:border-sky-900/40 p-5 rounded-2xl text-left">
                           <p className="text-[8.5px] font-black text-sky-600 dark:text-sky-400 uppercase tracking-wider mb-1">Yape Real</p>
                           <p className="text-xl font-display font-black text-sky-900 dark:text-sky-200 leading-none">S/ {yape.toFixed(2)}</p>
                         </div>
                         <div className="bg-brand-50/50 dark:bg-brand-950/30 border border-brand-100 dark:border-brand-900/40 p-5 rounded-2xl text-left">
                           <p className="text-[8.5px] font-black text-brand-600 dark:text-brand-400 uppercase tracking-wider mb-1">Cobros Créditos</p>
                           <p className="text-xl font-display font-black text-brand-900 dark:text-brand-200 leading-none">S/ {customerPaymentsToday.toFixed(2)}</p>
                         </div>
                         <div className="bg-rose-50/50 dark:bg-rose-950/30 border border-rose-100 dark:border-rose-900/40 p-5 rounded-2xl text-left">
                           <p className="text-[8.5px] font-black text-rose-600 dark:text-rose-400 uppercase tracking-wider mb-1">Ventas p. Cobrar</p>
                           <p className="text-xl font-display font-black text-rose-900 dark:text-rose-200 leading-none">S/ {creditoVendido.toFixed(2)}</p>
                           <p className="text-[7.5px] text-rose-450 dark:text-rose-400 font-bold uppercase mt-1 leading-none">* Pendiente de cobro</p>
                         </div>
                      </div>

                      {/* Recaudado Total */}
                      <div className="bg-slate-950 dark:bg-slate-950 border border-slate-800 p-6 md:p-8 rounded-3xl text-center shadow-lg relative overflow-hidden">
                         ...
                         <p className="text-[8.5px] text-slate-500 dark:text-slate-400 font-bold uppercase mt-4.5">
                      ...
                      <div className="space-y-3">
                         ...
                         <div className="bg-slate-50 dark:bg-slate-850 border border-slate-150 dark:border-slate-800 p-4 rounded-xl space-y-2.5">
                            ...
                            <div className="flex justify-between text-xs">
                               <span className="text-slate-500 dark:text-slate-400 font-black uppercase tracking-tight text-[10px]">Órdenes Canceladas:</span>
                               <span className="font-sans font-black text-emerald-600 dark:text-emerald-400">{salesToday.filter(o => o.estado === 'PAGADO').length}</span>
                            </div>
                            <div className="flex justify-between text-xs">
                               <span className="text-slate-500 dark:text-slate-400 font-black uppercase tracking-tight text-[10px]">Órdenes en Cuenta Corriente:</span>
                               <span className="font-sans font-black text-brand-600 dark:text-brand-400">{salesToday.filter(o => o.estado === 'CREDITO').length}</span>
                            </div>
                         </div>
                      </div>
                    </div>
              ...
              <div className="p-6 md:p-8 pt-0">
                 <button 
                  onClick={() => setShowReport(false)}
                  className="w-full py-4 bg-slate-900 dark:bg-slate-800 border-b-2 border-slate-950 dark:border-slate-700 text-white rounded-2xl font-black uppercase tracking-widest text-[10.5px] hover:bg-slate-850 active:scale-98 transition-all shadow-md shadow-slate-100 dark:shadow-none cursor-pointer"
                 >
